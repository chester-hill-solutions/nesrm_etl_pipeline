-- Street sort in load_contact_list_page_ids: order by b.street_address only so DBs without
-- street_name_sort_key / street_civic_sort_int / street_unit_sort_key still work.
-- (Walk-order UX uses lexical address sort until generated columns exist.)
  ...
  LOOP
    ...
    IF v_column = 'last_request' THEN
      ...
      v_order := v_order || format(
        'lr_%s.created_at %s NULLS %s, b.street_address ASC NULLS LAST',
        v_idx,
        upper(v_order_dir),

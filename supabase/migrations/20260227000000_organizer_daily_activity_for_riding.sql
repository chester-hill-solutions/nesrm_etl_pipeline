-- Organizer streak leaderboard: daily activity series per organizer tag (optional riding + date window).
--
-- We attribute survey activity to an organizer tag when:
-- - the contact is attributed to that organizer via contact.organizer (CSV of tags), AND
-- - the survey instance was sent by a profile whose profiles.organizer_tag matches that organizer tag (case-insensitive)
--
-- Returned dates are bucketed by UTC day to avoid timezone shifts.

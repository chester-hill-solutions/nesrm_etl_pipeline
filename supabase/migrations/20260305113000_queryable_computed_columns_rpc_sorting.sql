-- Support queryable computed custom columns in RPC sorting.
-- Computed-column filters still use fallback loader until resolve_filter_node
-- is migrated to the same backing source.

CREATE OR REPLACE VIEW public.contact_queryable_custom_fields AS
SELECT
  ccf.contact_id,
  ccf.column_id,
  ccf.value_text,
...
JOIN public.custom_columns cc
  ON cc.id = ccf.column_id
WHERE cc.column_kind = 'stored'
...
SELECT
  ...
  ccf.value_boolean,
  ccf.updated_at
FROM public.contact_computed_fields ccf
JOIN public.custom_columns cc
  ON cc.id = ccf.column_id
WHERE cc.column_kind = ANY(ARRAY['formula'::text, 'answer'::text, 'submission'::text]);

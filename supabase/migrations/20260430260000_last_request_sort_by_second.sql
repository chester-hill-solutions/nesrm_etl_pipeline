-- Truncate last_request to whole seconds so batch updates with the same wall time tie-break
-- by street walk order (name, civic, unit) instead of microsecond / id order.
--
-- GENERATED STORED requires an immutable expression; date_trunc(timestamptz) is STABLE, so use
-- a plain column maintained by trigger (same value as date_trunc('second', created_at)).

ALTER TABLE public.request
  ADD COLUMN IF NOT EXISTS created_at_second timestamptz;

COMMENT ON COLUMN public.request.created_at_second IS
  'created_at truncated to seconds; aligns list sort with batch wall-clock ties.';

CREATE OR REPLACE FUNCTION public.request_set_created_at_second()
RETURNS trigger
LANGUAGE plpgsql
...
BEGIN
  IF NEW.created_at IS NULL THEN
    NEW.created_at_second := NULL;
  ELSE
    NEW.created_at_second := date_trunc('second', NEW.created_at);
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS request_set_created_at_second ON public.request;
CREATE TRIGGER request_set_created_at_second
  BEFORE INSERT OR UPDATE OF created_at ON public.request
  FOR EACH ROW
  EXECUTE FUNCTION public.request_set_created_at_second();

UPDATE public.request
SET created_at_second = date_trunc('second', created_at)
WHERE created_at_second IS DISTINCT FROM date_trunc('second', created_at);

CREATE INDEX IF NOT EXISTS request_created_at_second_idx
  ON public.request (created_at_second);
  ...
  LOOP
    ...
    IF v_column = 'last_request' THEN
      ...
      v_order := v_order || format(
        'lr_%s.created_at_second %s NULLS %s, public.contact_street_name_sort_key(b.street_address) ASC NULLS LAST, public.contact_street_civic_sort_int(b.street_address) ASC NULLS LAST, public.contact_street_unit_sort_key(b.street_address) ASC NULLS LAST',
        v_idx,
        upper(v_order_dir),
    ...
    ELSIF v_column = 'street_address' THEN
      ...
      IF v_street_mode = 'name' THEN
        v_order := v_order || format(
          'public.contact_street_name_sort_key(b.street_address) %s NULLS %s, public.contact_street_civic_sort_int(b.street_address) %s NULLS %s',
          upper(v_order_dir),
          CASE WHEN v_order_dir = 'asc' THEN 'LAST' ELSE 'FIRST' END,
          upper(v_order_dir),
          CASE WHEN v_order_dir = 'asc' THEN 'LAST' ELSE 'FIRST' END
      ...
      ELSIF v_street_mode = 'civic' THEN
        v_order := v_order || format(
          'public.contact_street_civic_sort_int(b.street_address) %s NULLS %s, public.contact_street_name_sort_key(b.street_address) %s NULLS %s',
          upper(v_order_dir),
          CASE WHEN v_order_dir = 'asc' THEN 'LAST' ELSE 'FIRST' END,
          upper(v_order_dir),
          CASE WHEN v_order_dir = 'asc' THEN 'LAST' ELSE 'FIRST' END
      ...
      ELSIF v_street_mode = 'unit' THEN
        v_order := v_order || format(
          '(CASE WHEN public.contact_street_unit_sort_key(b.street_address) ~ %L THEN substring(public.contact_street_unit_sort_key(b.street_address) from 2)::integer ELSE NULL END) %s NULLS %s, public.contact_street_unit_sort_key(b.street_address) %s NULLS %s, public.contact_street_civic_sort_int(b.street_address) %s NULLS %s, public.contact_street_name_sort_key(b.street_address) %s NULLS %s',
          '^#[0-9]+$',
          upper(v_order_dir),
          CASE WHEN v_order_dir = 'asc' THEN 'LAST' ELSE 'FIRST' END,
          upper(v_order_dir),
      ...
      ELSE
        v_order := v_order || format(
          'public.contact_street_name_sort_key(b.street_address) %s NULLS %s, public.contact_street_civic_sort_int(b.street_address) %s NULLS %s, public.contact_street_unit_sort_key(b.street_address) %s NULLS %s',
          upper(v_order_dir),
          CASE WHEN v_order_dir = 'asc' THEN 'LAST' ELSE 'FIRST' END,
          upper(v_order_dir),
          CASE WHEN v_order_dir = 'asc' THEN 'LAST' ELSE 'FIRST' END,

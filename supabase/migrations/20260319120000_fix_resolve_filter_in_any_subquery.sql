-- Fix: survey pseudo-column "in" filters used scalar = ANY(SELECT array_agg(...)),
-- which returns one text[] row and errors with "operator does not exist: text = text[]".
-- Use IN (SELECT trim from unnest) instead.

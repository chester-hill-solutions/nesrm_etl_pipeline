-- Survey starts heatmap (per sender profile).
-- Attribution: survey_instances.sent_by (profiles.id)
-- Timestamp: survey_responses.started_at (fallback to created_at)

CREATE OR REPLACE FUNCTION "public"."get_survey_starts_heatmap_for_sender"(
  "p_sent_by" uuid,
  "p_days" int DEFAULT 365,
  "p_tz" text DEFAULT 'UTC'
)
RETURNS TABLE("date" date, "count_started" bigint)
LANGUAGE "sql"
STABLE
AS $$
  WITH params AS (
  ...
  counts AS (
    SELECT
      (COALESCE(sr.started_at, sr.created_at) AT TIME ZONE bounds.tz)::date AS d,
      COUNT(*)::bigint AS count_started
    FROM bounds
    ...
    WHERE sr.status IN ('in_progress', 'completed')
      AND (COALESCE(sr.started_at, sr.created_at) AT TIME ZONE bounds.tz)::date
        BETWEEN bounds.start_date AND bounds.end_date
    GROUP BY 1
  )
  SELECT
    calendar_days.d AS date,
    COALESCE(counts.count_started, 0)::bigint AS count_started
  FROM calendar_days

-- Street sort: name then civic then unit via IMMUTABLE functions (same as generated columns).
-- Uses public.contact_street_* functions so ORDER BY does not require those columns on contact.
  ...
  LOOP
    ...
    ELSIF v_column = 'street_address' THEN
      ...
      IF v_street_mode = 'name' THEN
        v_order := v_order || format(
          'public.contact_street_name_sort_key(b.street_address) %s NULLS %s',
          upper(v_order_dir),
          CASE WHEN v_order_dir = 'asc' THEN 'LAST' ELSE 'FIRST' END
        );
      ELSIF v_street_mode = 'civic' THEN
        v_order := v_order || format(
          'public.contact_street_civic_sort_int(b.street_address) %s NULLS %s',
          upper(v_order_dir),
          CASE WHEN v_order_dir = 'asc' THEN 'LAST' ELSE 'FIRST' END
        );
      ELSIF v_street_mode = 'unit' THEN
        v_order := v_order || format(
          'public.contact_street_unit_sort_key(b.street_address) %s NULLS %s',
          upper(v_order_dir),
          CASE WHEN v_order_dir = 'asc' THEN 'LAST' ELSE 'FIRST' END
        );
      ELSE

-- Household grouping: normalized keys + extend load_contact_list_page_ids.
-- When p_group_by_household is true, reorders the fully sorted candidate list so transitive
-- matches (email / phone / address) are adjacent, then paginates. Clustering runs only
-- over p_contact_ids (same cap semantics as callers).

CREATE OR REPLACE FUNCTION public.contact_household_email_key(p_email text)
RETURNS text
LANGUAGE sql
IMMUTABLE
PARALLEL SAFE
...
AS $f$
  SELECT CASE
    WHEN p_email IS NULL OR btrim(p_email) = '' THEN NULL::text
    WHEN strpos(lower(btrim(p_email)), '@') < 2 THEN NULL::text
    ELSE (
      SELECT CASE
        WHEN s.domain_part = ''::text THEN NULL::text
        ELSE lower(split_part(s.local_part, '+', 1) || '@' || s.domain_part)
      END
      FROM (
        SELECT
          split_part(lower(btrim(p_email)), '@', 1) AS local_part,
          split_part(lower(btrim(p_email)), '@', 2) AS domain_part
      ) s
    )
  END;
$f$;

COMMENT ON FUNCTION public.contact_household_email_key(text) IS
  'Lowercase email with +tag stripped from local part (first + only); used for household matching.';

CREATE OR REPLACE FUNCTION public.contact_household_phone_key(p_phone text)
RETURNS text
LANGUAGE sql
IMMUTABLE
PARALLEL SAFE
...
AS $f$
  SELECT CASE
    ...
    ELSE (
      SELECT CASE
        WHEN length(x.d) < 10 THEN NULL::text
        WHEN length(x.d) = 11 AND left(x.d, 1) = '1' THEN substr(x.d, 2, 10)
        ELSE x.d
      END
      FROM (SELECT regexp_replace(btrim(p_phone), '\D', '', 'g') AS d) x
    )
  END;
$f$;

COMMENT ON FUNCTION public.contact_household_phone_key(text) IS
  'Digits-only phone; strips leading 1 for 11-digit NA numbers; min 10 digits; household matching.';

CREATE OR REPLACE FUNCTION public.contact_household_address_key(
  p_street text,
  p_municipality text,
  p_postcode text
)
...
AS $f$
  SELECT CASE
    WHEN p_street IS NULL OR btrim(p_street) = '' THEN NULL::text
    ELSE lower(
      regexp_replace(
        concat_ws(
          '|',
          btrim(p_street),
          nullif(btrim(COALESCE(p_municipality, '')), ''),
...
COMMENT ON FUNCTION public.contact_household_address_key(text, text, text) IS
  'Normalized composite address when street is non-empty; used for household matching.';

DROP FUNCTION IF EXISTS public.load_contact_list_page_ids(bigint[], jsonb, integer, integer, text);
...
COMMENT ON FUNCTION public.load_contact_list_page_ids(bigint[], jsonb, integer, integer, text, boolean) IS
  'Returns ordered contact IDs, total_count, and optional household_group_id (min contact id in household component when p_group_by_household). '
  'When grouping: clusters by normalized email (+tag strip), phone (digits, NA 1-prefix), and address (street required) over p_contact_ids, '
  'reorders so household members are adjacent, then applies pagination.';

-- Street sort tiebreakers: name mode adds civic (integer 20 < 100); civic mode adds name;
-- unit mode orders numeric #suites by integer before lexicographic unit text, then civic/name.
  ...
  LOOP
    ...
    IF v_column = 'last_request' THEN
      ...
      v_order := v_order || format(
        'lr_%s.created_at %s NULLS %s, public.contact_street_name_sort_key(b.street_address) ASC NULLS LAST, public.contact_street_civic_sort_int(b.street_address) ASC NULLS LAST, public.contact_street_unit_sort_key(b.street_address) ASC NULLS LAST',
        v_idx,
        upper(v_order_dir),

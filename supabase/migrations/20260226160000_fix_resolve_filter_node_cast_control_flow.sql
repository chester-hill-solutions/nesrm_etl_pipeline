-- Fix: resolve_filter_node typed casts must not fall through to uncasted queries.
-- The previous implementation executed a casted query and then also executed the uncasted
-- version, re-triggering "operator does not exist: bigint = text" for columns like contact.id.

-- Feature: support segment membership column in resolve_filter_node.
...
BEGIN
  ...
  IF v_col = 'segment_id' THEN
    ...
    IF v_op = 'eq' THEN
      RETURN QUERY
      SELECT csm.contact_id
      FROM public.contact_segment_membership csm
      WHERE csm.segment_id = v_segment_id;
      RETURN;
    ELSIF v_op = 'neq' THEN
      RETURN QUERY
      SELECT c.id
  ...
  IF v_col = 'survey_status' THEN
    IF v_op = 'eq' THEN
      ...
      WHERE (c.contact_status IS NULL OR c.contact_status <> 'archived')
        AND COALESCE(l.status, 'not_started') = v_val;
      RETURN;
    ELSIF v_op = 'neq' THEN
      RETURN QUERY
      WITH latest AS (
    ...
    ELSIF v_op = 'contains' THEN
      ...
      LEFT JOIN latest l ON l.contact_id = c.id
      WHERE (c.contact_status IS NULL OR c.contact_status <> 'archived')
        AND COALESCE(l.status, 'not_started') ILIKE ('%' || v_esc_ilike || '%');
      RETURN;
    ELSIF v_op = 'not_contains' THEN
      ...
      LEFT JOIN latest l ON l.contact_id = c.id
      WHERE (c.contact_status IS NULL OR c.contact_status <> 'archived')
        AND COALESCE(l.status, 'not_started') NOT ILIKE ('%' || v_esc_ilike || '%');
      RETURN;
    ...
    ELSIF v_op = 'in' THEN
      ...
      LEFT JOIN latest l ON l.contact_id = c.id
      WHERE (c.contact_status IS NULL OR c.contact_status <> 'archived')
        AND COALESCE(l.status, 'not_started') = ANY(
          (
            SELECT COALESCE(array_agg(TRIM(t)), ARRAY[]::text[])
            FROM unnest(string_to_array(COALESCE(v_val, ''), ',')) t
            WHERE TRIM(t) <> ''

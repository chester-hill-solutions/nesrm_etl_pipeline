-- Fix: use named record fields (ids, total_count) instead of f1/f2 so the
-- record populated from "SELECT a.ids, t.total_count FROM agg_ids a, tot t"
-- unpacks correctly. Resolves 42703 "record has no field f1".
...
BEGIN
  ...
  WHERE (c.contact_status IS NULL OR c.contact_status <> 'archived')
    AND (
      ...
      OR (p_scope_type = 'signups' AND p_scope_values IS NOT NULL AND cardinality(p_scope_values) > 0 AND (
        EXISTS (
          SELECT 1 FROM unnest(string_to_array(regexp_replace(lower(COALESCE(c.organizer, '')), '[,;\n]+', ',', 'g'), ',')) AS t(token)
          WHERE TRIM(t.token) <> '' AND TRIM(t.token) IN (SELECT TRIM(x) FROM unnest(p_scope_values) AS x WHERE TRIM(x) <> '')
        ) OR EXISTS (SELECT 1 FROM unnest(p_scope_values) AS s(val) WHERE TRIM(s.val) <> '' AND lower(COALESCE(c.organizer, '')) LIKE '%' || lower(TRIM(s.val)) || '%')
      ))
    );
  ...
  WITH ordered AS (
    ...
    FROM public.load_contact_list_page_ids(v_candidate_ids, COALESCE(p_sort_rules, '[]'::jsonb), v_page_num, v_limit) o
  ),

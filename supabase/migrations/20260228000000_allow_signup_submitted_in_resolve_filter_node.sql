-- Fix: allow boolean contact fields like signup_submitted in resolve_filter_node.
-- Without allowlisting + boolean casts, filter groups that include signup_submitted collapse to no matches,
-- which breaks segments like "Ready for Submission" that rely on signup_submitted != true.
        ...
        RETURN QUERY EXECUTE format(
          'SELECT c.id
           ...
           WHERE (c.contact_status IS NULL OR c.contact_status <> ''archived'')
             AND c.%I = ANY(
               (
                 SELECT COALESCE(array_agg((TRIM(t))::%s), ARRAY[]::%s[])
                 FROM unnest(string_to_array(COALESCE($1, ''''), '','')) t
                 WHERE TRIM(t) <> ''''
               )
             )',
          v_db_col,

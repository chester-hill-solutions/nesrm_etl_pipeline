-- Spaced unit letter after civic (e.g. "123 A Main St" -> sort key "Main St").
-- Preserves literal street names like "A Street" (single letter + "Street").
...
BEGIN
  ...
  LOOP
    ...
    ELSIF s ~ '^[0-9]{3,}[[:upper:]]' THEN
      s := trim(regexp_replace(s, '^[0-9]+', '', ''));
    ELSIF s ~ '^[A-Za-z][[:space:]]+' AND s !~* '^[A-Za-z][[:space:]]+street([[:space:]]|,|$)' THEN
      s := trim(regexp_replace(s, '^[A-Za-z][[:space:]]+', '', ''));
    ELSE
      EXIT;
    END IF;
...
COMMENT ON FUNCTION public.contact_street_name_sort_key(text) IS
  'Sort key from street_address: strips civic tokens and spaced unit letters (e.g. 123 A Main); preserves A Street-style names and ordinal streets.';

-- Fix last_request sort: use request.created_at for the request referenced by
-- contact.last_request (FK contact_last_request_fkey), not MAX(request.created_at)
-- where request.contact_id = contact.id (request.contact_id can be null).
...
BEGIN
  ...
  LOOP
    ...
    IF v_column = 'last_request' THEN
      -- Sort by created_at of the request row referenced by contact.last_request (FK),
      -- not by request.contact_id (which can be null).
      v_joins := v_joins || format(
        ' LEFT JOIN public.request lr_%s ON lr_%s.id = b.last_request',
        v_idx,
        v_idx
      );
...
COMMENT ON FUNCTION public.load_contact_list_page_ids(bigint[], jsonb, integer, integer) IS
  'Returns ordered contact IDs and total_count for a pre-filtered contact ID set using server-side sort rules (survey_status, custom columns, last_request by FK request.created_at, and standard columns).';

-- RPC + index additions for survey-loading performance.
-- - Adds a dedicated survey modal list RPC (surveys + has_in_progress_response).
-- - Adds scoped filter resolver + scoped contact dashboard payload RPC.
-- - Adds composite indexes for latest-per-survey and answers joins.

CREATE INDEX IF NOT EXISTS idx_survey_responses_contact_survey_created_id_active
  ON public.survey_responses (contact_id, survey_id, created_at DESC, id DESC)
  WHERE status IN ('completed', 'in_progress');

CREATE INDEX IF NOT EXISTS idx_survey_response_answers_question_response
  ON public.survey_response_answers (question_id, response_id);
  ...
  WITH candidate_surveys AS (
    ...
    WHERE s.status = 'active'
      ...
      AND (
        ...
        OR cardinality(s.visible_to_roles) = 0
        OR p_role = ANY(s.visible_to_roles::text[])
      )
  ),
  latest AS (
...
BEGIN
  ...
  IF v_has_filter THEN
    v_candidate_ids := COALESCE(
      ARRAY(SELECT * FROM public.resolve_filter_group_scoped(p_filter_group, v_candidate_ids)),
      ARRAY[]::bigint[]
    );

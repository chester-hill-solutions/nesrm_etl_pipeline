-- Add a payload-derived has_message flag and allow filtering via get_web_requests RPC.
-- This keeps pagination/total_count accurate when filtering by "Has message" in the CRM dashboard.

-- 1) Safe JSON parsing helper for jsonb string fields.
...
-- 2) Extract message text using the same precedence as the UI:
-- Prefer payload.body (object, or stringified JSON object) when present; otherwise root payload.
-- Then select message/content/body/text; treat empty/whitespace as null.
CREATE OR REPLACE FUNCTION public.get_request_message_text(p_payload jsonb)
RETURNS text
LANGUAGE plpgsql
IMMUTABLE
SET search_path TO 'public'
...
DECLARE
  ...
  candidate text;
BEGIN
  IF p_payload IS NULL OR p_payload::text = 'null' THEN
    RETURN NULL;
  END IF;

  root := p_payload;
  IF jsonb_typeof(root) <> 'object' THEN
    RETURN NULL;
  END IF;

  body := root->'body';

  IF body IS NOT NULL AND jsonb_typeof(body) = 'object' THEN
    body_obj := body;
  ELSIF body IS NOT NULL AND jsonb_typeof(body) = 'string' THEN
    body_obj := public.try_parse_jsonb(body #>> '{}');
    IF body_obj IS NOT NULL AND jsonb_typeof(body_obj) <> 'object' THEN
      body_obj := NULL;
    END IF;
  ELSE
    body_obj := NULL;
...
CREATE OR REPLACE FUNCTION public.get_request_has_message(p_payload jsonb)
RETURNS boolean
LANGUAGE plpgsql
IMMUTABLE
SET search_path TO 'public'
AS $$
BEGIN
  RETURN public.get_request_message_text(p_payload) IS NOT NULL;
END;
$$;

-- 3) Stored generated column on request.
ALTER TABLE public.request
  ADD COLUMN IF NOT EXISTS has_message boolean GENERATED ALWAYS AS (
    public.get_request_has_message(payload)
  ) STORED;

-- 4) Upgrade get_web_requests RPC to support filter_has_message.
-- Drop the existing show_duplicates overload to avoid PostgREST ambiguity when adding a param.
...
CREATE OR REPLACE FUNCTION public.get_web_requests(
  ...
  filter_has_message boolean DEFAULT NULL
)
RETURNS TABLE(
  id bigint,
...
BEGIN
  ...
  IF filter_has_message IS NOT NULL THEN
    base_query := base_query || format(' AND r.has_message = %L', filter_has_message);
  END IF;
...
-- Keep the legacy 10-arg overload in sync by delegating to the 12-arg function.
CREATE OR REPLACE FUNCTION public.get_web_requests(
  filter_success boolean DEFAULT NULL,
  filter_status text DEFAULT NULL,
  filter_category text DEFAULT NULL,

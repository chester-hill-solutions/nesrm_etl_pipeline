-- Avoid redundant contact re-joins in the optimized dashboard payload RPC
-- when no search term is present. This keeps the no-search path on the
-- filtered ID set instead of performing tens of thousands of PK lookups.
...
BEGIN
  ...
  IF p_search_term IS NULL OR TRIM(p_search_term) = '' THEN
    IF v_has_filter THEN
      ...
      scoped_id_array AS (
        SELECT COALESCE(array_agg(s.id ORDER BY s.id), ARRAY[]::bigint[]) AS ids
        FROM scoped_ids s
      ),
      filtered_ids AS (
        SELECT f.id
        FROM public.resolve_filter_group_scoped(
          p_filter_group,
          COALESCE((SELECT ids FROM scoped_id_array), ARRAY[]::bigint[])
        ) AS f(id)
      ),
      total_count AS (
  ...
  WITH ordered AS (
    SELECT o.contact_id, row_number() OVER () AS rn
  ...
  SELECT a.ids INTO v_page_rec FROM agg_ids a;
